...
![版本](https://img.shields.io/badge/version-1.2.0-blue) ![许可证](https://img.shields.io/badge/license-MIT-green) ![平台](https://img.shields.io/badge/platform-Windows%2010%2F11-lightgrey)
...
- **心流专注**：任务卡片 ▶ 或抽屉「开始专注」一键计时，同一时间仅一个计时；托盘 tooltip 显示「正在专注：X (mm:ss)」；番茄钟 25/5、45/10、50/10 档位，到点自动停表并弹通知，休息结束提醒继续；无任务专注模式（冥想/读书/运动/写作/听播客/散步）点击即计时。
- **时光月历**：月历圆点标记（任务=优先级色），当日详情面板就地增改日程与任务，一键打开/创建当日速记。
- **习惯打卡**：双列卡片网格，卡片右侧圆形打卡按钮；强度评分移植 Loop Habit Tracker 开源 EWMA 算法，详情为本周/本月/本年次数与连续/强度统计块，可设每日提醒时刻（当日未打卡才提醒）；打卡有弹跳与彩纸粒子，连续 7/21/66/100/365 天里程碑 toast，全部打卡完成有彩蛋；常用习惯（骑行/步行/喝水/午休/阅读/冥想）一键速建。
